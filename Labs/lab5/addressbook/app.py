...
def home():
    if request.method == 'POST':
        ...
        db = DB()
        user = User('', '')
        user = db.login(email=email, password=generate_password_hash(
...
def sign_up():
    if request.method == 'POST':
        ...
        if len(email) < 4:
            ...
            check = db.register_user(email=email, password=generate_password_hash(
                password1, method='sha256'))
            if(check):
                id = db.getUserId(email=email, password=generate_password_hash(
                password1, method='sha256'))
                session["email"] = email
                session["id"] = id
                session["password"] = password=generate_password_hash(
                password1, method='sha256')
